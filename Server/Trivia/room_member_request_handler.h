...
{
public:
	RoomMemberRequestHandler(const int& roomId, const LoggedUser& user, std::shared_ptr<RoomManager> roomManager, std::shared_ptr<RequestHandlerFactory> handlerFactory);
...
private:
	RequestResult leaveRoom(const Request& req) const;
